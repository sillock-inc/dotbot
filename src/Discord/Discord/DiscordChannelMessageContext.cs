...
public class DiscordChannelMessageContext : IDiscordChannelMessageContext
{
    ...
    public async Task ReplyAsync(MessageComponent msg)
    {
        var msgRef = new MessageReference(_message.Id);
        await _message.Channel.SendMessageAsync("Here are the matching commands", false, null, RequestOptions.Default, AllowedMentions.All, msgRef, msg);
    }

    public async Task ReplyFileAsync(string fileName, Stream fs)
    {
        var msgRef = new MessageReference(_message.Id);

        await _message.Channel.SendFileAsync(fs, fileName, messageReference: msgRef);
    }
